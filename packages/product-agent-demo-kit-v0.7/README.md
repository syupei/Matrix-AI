...
1. 将压缩包解压到一个新的独立目录；建议不要放在原讨论项目内部。直接将解压得到的 `product-agent-demo-kit-v0.7` 文件夹作为新项目根目录。这样会保留隐藏目录 `.agents`。
...
保留 `product/`、确认快照和观察记录。更新本包的 skill 文件夹；如自行修改过 skill，先比较并合并。根目录 AGENTS.md 也需合并新的共用成果与接力要求，保留已有连续执行规则。然后使用 START.md 中的升级语，让 Agent 根据实际成果和确认依据恢复当前位置、补齐阶段概览，并展示当前动作。不从头清空或重做项目，不把未知历史进度补成已完成。v0.7 沿用 v0.5 的表单（版本仍为 0.5）；旧版输入与已有效确认仍保留，不将旧版“请建议”追认为新委托。
...
- 本包工作指引：v0.7，整理日期 2026-09-17，依据用户关于人机共同阅读、编辑、监督与逐步确认的要求，以及本次讨论的七环节草案。
...
本次修改成果组织、模板、角色接力和迁移规则，保留首轮澄清、阶段可见性、真实审阅与连续执行要求。交付前检查 skill 格式、运行指引链接、48 项能力、源材料与 HTML 表单不变、ZIP 解压及哈希；检查正文引用、角色职责和迁移规则的一致性。未运行新版完整多轮 demo 或真实下游交接，需在试验项目检验能否按入口找到唯一规则、QA 是否可据此设计验证，以及旧确认能否正确继承。
