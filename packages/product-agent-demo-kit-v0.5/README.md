...
1. 将压缩包解压到一个新的独立目录；建议不要放在原讨论项目内部。直接将解压得到的 `product-agent-demo-kit-v0.5` 文件夹作为新项目根目录。这样会保留隐藏目录 `.agents`。
...
例如，Agent 应说明“现在请确认目标与首期范围”，指出具体需要检查的决定，并让用户可以直接说同意、提出修改或要求解释。无需记编号、寻找文件或猜测“继续”是什么意思。如果前一轮遗漏了审阅入口，Agent 应先补齐提示，不能将用户的“请继续”追认为对未展示成果的批准。
...
保留 `product/`、确认快照和观察记录。更新本包的 skill 文件夹；如自行修改过 skill，先比较并合并。根目录 AGENTS.md 也需合并新增的阶段定位要求。然后使用 START.md 中的升级语，让 Agent 根据实际成果和确认依据恢复当前位置、补齐阶段概览，并展示当前动作。不从头清空或重做项目，不把未知历史进度补成已完成。v0.5 表单版本为 0.5；旧版输入与已有效确认仍保留，不将旧版“请建议”追认为新委托。
...
- 本包工作指引：v0.5，整理日期 2026-09-17，依据用户关于人机共同阅读、编辑、监督与逐步确认的要求，以及本次讨论的七环节草案。
...
交付前检查 skill 格式、运行指引引用、48 项能力完整性、原文切片和源文件一致性、压缩包完整性与可迁移路径。v0.5 修改澄清分类、风格指引及 HTML 表单，能力标准保持不变。已在隔离浏览器检查无预选、核心信息提醒、自定义补充、文案视觉独立选择、委托范围、品牌规范保留、隐藏字段不导出、文本安全、Markdown/JSON 导出与移动端无横向溢出，并查看显示截图。文件和表单检查不证明模型的真实澄清判断能力；本次未运行新版完整多轮 demo，需在新项目复验必需项判断、委托边界及风格转译。
