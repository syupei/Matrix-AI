...
产品经理按独立专业 Agent 承担产品责任，product-agent skill 是岗位配置；安装与加载
不等于实例已运行。共用 [内容设计与审读](.agents/skills/content-design-review/SKILL.md)
由产品/设计在成稿、扩展和交接时使用，项目表达标准留在 STYLE，具体成稿保持唯一来源。
整组标题、正文、按钮在信息完整前提下尽量精简；实际文字与语境回读后才能记录检查结果。
一个设计 Agent 贯穿 UX 和视觉/完整 UI，默认一套推荐，明确要求才出多方案；保留粗结构、
关键交互与代表视觉合并、完整设计三个审阅节点。目录重组及项目偏好目录仍暂缓。
所有角色的用户确认与多步流程显示执行 [统一交互规则](collaboration/USER-INTERACTION.md)：
确认节点实际提供“确认，可继续”或“需要进行调整，进入调整对话”交互选项；
流程明确显示“共 N 步，当前进行第 X 步”，项目/角色两层分开，不用步骤位置冒充完成率。
保留已有授权、集中人审与自然语言回应；无可用控件时明确说明并回退文本，预选不算确认。
角色启动/续做/规范变更时读取各自 skill 内的 GUIDANCE-INDEX.md，按
[指引使用与项目调整](collaboration/GUIDANCE-RESOLUTION.md) 采用有效引用及用户说明。
设计默认主动扫描动态/触觉/声音反馈机会，选择、制作并核对适用响应；明确停用或替代风格优先。
同名索引位于现有 skill 内，目录重组与独立偏好目录仍暂缓。
产品讨论的描述性名称默认暂称，按产品命名指引设计和交接有效名称/状态；
真实品牌指代、明确命名确认和稳定概念ID保持，不因功能确认自动锁定名称。
'''+END+'\n'
